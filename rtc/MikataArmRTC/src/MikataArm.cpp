...
MikataArm::MikataArm(RTC::Manager* manager)
  ...
  : RTC::DataFlowComponentBase(manager),
    m_JointStatesOut("JointStates", m_JointStates),
...
RTC::ReturnCode_t MikataArm::onInitialize()
{
  ...
  // Set OutPort buffer
  addOutPort("JointStates", m_JointStatesOut);
...
{
  m_JointStates.data.length(7);
  std::vector<ssr::mikata::JointInfo> jointInfos = m_pArm->jointInfos();
  ssr::mikata::JointInfo gripper = m_pArm->gripper();
  for(int i = 0;i < 6;i++) {
    m_JointStates.data[i] = jointInfos[i].angle;
  }
  m_JointStates.data[6] = gripper.angle;
// m_pArm.getFeedbackPosJoint
  m_JointStatesOut.write();
  return RTC::RTC_OK;
}
